...
if errorlevel 1 (
    ...
    powershell -ExecutionPolicy Bypass -Command ^
        "$ErrorActionPreference = 'SilentlyContinue'; ^
         $nodeUrl1 = 'https://npmmirror.com/mirrors/node/v22.14.0/node-v22.14.0-win-x64.zip'; ^
         $nodeUrl2 = 'https://mirrors.huaweicloud.com/nodejs/v22.14.0/node-v22.14.0-win-x64.zip'; ^
         $nodeUrl3 = 'https://mirrors.cloud.tencent.com/nodejs-release/v22.14.0/node-v22.14.0-win-x64.zip'; ^
         $nodeUrl4 = 'https://nodejs.org/dist/v22.14.0/node-v22.14.0-win-x64.zip'; ^
         $destDir = '%NODE_DIR%'; ^
         $zipFile = Join-Path $destDir 'node.zip'; ^
         New-Item -ItemType Directory -Force -Path $destDir | Out-Null; ^
         $urls = @($nodeUrl1, $nodeUrl2, $nodeUrl3, $nodeUrl4); ^
         $success = $false; ^
         foreach ($url in $urls) { ^
             Write-Host "  尝试下载: $url"; ^
             try { ^
